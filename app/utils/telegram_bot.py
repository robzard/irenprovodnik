...
async def send_telegram_message_succeeded(payment: Payments):
    ...
    link = await bot.create_chat_invite_link(payment.user_id, member_limit=1)

    try:
        if payment.status == "succeeded":
            text = (f'{description_type[payment.description]} на сумму {payment.amount} рублей прошла успешно!\n'
                    f'Подписка на приватный канал оформлена, вы уже добавлены в канал, перейдите по ссылке - {link}.\n\n'
